...
| — market evidence integrity | — | 2026-08-29 (Profit Scanner v2): HOT/LIST/SKIP no longer depends on sell-through rate, days-to-sell, or demand level — decision authority is `netProfit`/`roi`/`evidenceQuality` only, evaluated per-marketplace by a new opportunity engine (eBay live; Etsy/Reverb/Discogs/Poshmark/Mercari/Amazon are explicit not-configured placeholders). Weak/no evidence surfaces as `LIMITED EVIDENCE` (`decisionAvailable:false`) rather than a capped-to-LIST decision. A failed eBay Browse lookup still never counts as a verified zero-active-listing result (`ebayBrowse.ts` returns `null`, never a fabricated `matchingActiveCount: 0`). See `docs/HANDOFF.md` 2026-08-29 entry. Release B (real comp-matching), C (multi-stage identification), D (SoldComps pagination correctness) from the earlier remediation plan are still not done. |
