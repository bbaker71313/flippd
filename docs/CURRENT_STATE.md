...
- **Verified market-data pipeline (2026-08-26) is built but NOT live.** `packages/shared/src/utils/marketMetrics.ts` (+ Deno mirror) and `supabase/functions/_shared/{ebayAppAuth,ebayTaxonomy,ebayCatalog,ebayBrowse,soldCompsProvider,itemIdentification,marketDataPipeline}.ts` implement provider-agnostic identification + eBay Catalog/Taxonomy/Browse + SoldComps sold-history evidence, replacing the still-denied Marketplace Insights. Not called by `claude-proxy` — scans are still AI-estimate market data (`marketDataSource: 'ai_estimate'`), pending: `SOLDCOMPS_API_KEY` configuration + live contract verification, and product decisions on the sell-through-rate formula and demand-level thresholds. See `HANDOFF.md`.
...
| Date | Change |
|------|--------|
| 2026-08-26 | P0 market-data remediation: provider-agnostic identification interface + eBay Catalog/Taxonomy/Browse clients + SoldComps sold-history provider + deterministic price/turnover metrics built (`packages/shared` + `supabase/functions/_shared`), tested where pure logic. **Not wired into any live scan path** — blocked on `SOLDCOMPS_API_KEY`, live SoldComps contract verification, and product decisions on sell-through-rate formula + demand thresholds. See `HANDOFF.md`. |
