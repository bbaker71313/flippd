...
| — market evidence integrity | — | 2026-08-29 (Profit Scanner v2): HOT/LIST/SKIP no longer depends on sell-through rate, days-to-sell, or demand level — decision authority is `netProfit`/`roi`/`evidenceQuality` only, evaluated per-marketplace by a new opportunity engine (eBay live; Etsy/Reverb/Discogs/Poshmark/Mercari/Amazon are explicit not-configured placeholders). **Current live code** still surfaces weak/no evidence as `LIMITED EVIDENCE` (`decisionAvailable:false`) rather than a capped-to-LIST decision — but as of 2026-08-31 this is **no longer the approved product rule** (see `docs/files/DECISIONS.md`'s "Evidence ladder" decision): a reasonably identifiable item is meant to always reach HOT/LIST/SKIP via a weaker-evidence ladder, and only a genuine identification failure or a provider/system error may still withhold a decision. That change is not yet implemented — it is scoped as R2/R3 in `docs/files/PROFIT_SCANNER_IMPLEMENTATION_PLAN_2026-08-30.md` (Revision 3). A failed eBay Browse lookup still never counts as a verified zero-active-listing result (`ebayBrowse.ts` returns `null`, never a fabricated `matchingActiveCount: 0`). See `docs/HANDOFF.md` 2026-08-29 entry. Release B (real comp-matching), C (multi-stage identification), D (SoldComps pagination correctness) from the earlier remediation plan are still not done. |
...
- **Verified market-data pipeline is wired into single/text/shelf scans.** `packages/shared/src/utils/marketMetrics.ts` (+ Deno mirror) and `supabase/functions/_shared/{ebayAppAuth,ebayTaxonomy,ebayCatalog,ebayBrowse,soldCompsProvider,itemIdentification,marketDataPipeline}.ts` implement provider-agnostic identification + eBay Catalog/Taxonomy/Browse + verified sold-history evidence, replacing the still-denied Marketplace Insights. As of 2026-08-29, Trawl is the preferred sold provider when `TRAWL_API_KEY` is configured; SoldComps remains the configuration fallback. Both feed the same identity-aware comp qualification and deterministic metrics path. eBay Catalog remains best-effort because the current credentials are not entitled (403). When verification fails, no authoritative HOT/LIST/SKIP decision, profit/ROI, or max-buy-price is computed from an AI estimate. A real authenticated scan against the Trawl-backed deployment is still required before treating provider behavior as fully smoke-tested.
- **AI-market-authority gate (`resolveScanResultCore` in `supabase/functions/claude-proxy/index.ts`)** is now the single place that decides whether a scan's HOT/LIST/SKIP, net profit, ROI, and max-buy-price may be computed at all — only when `marketDataSource: 'verified'`. When verification fails, the response reports `decisionAvailable: false`, `decisionStatus: 'insufficient_market_data'`, every authoritative field `null`, and `aiEstimate:null`; AI-created numerical market values are no longer requested or displayed. `apps/web/public/scanResultContract.js` validates this shape; `app.html` renders a distinct no-recommendation state with a completed-listings search action. **This "no verification → no decision" behavior is being retired, not extended**: per the 2026-08-31 "Evidence ladder" decision (`docs/files/DECISIONS.md`), a reasonably identifiable item must reach HOT/LIST/SKIP via progressively weaker evidence rather than stop here — this gate's no-decision path should end up reachable only for a genuine identification failure or a provider/system error, once `docs/files/PROFIT_SCANNER_IMPLEMENTATION_PLAN_2026-08-30.md` Revision 3's R2/R3 ship.
