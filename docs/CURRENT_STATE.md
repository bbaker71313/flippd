...
- **AI-market-authority gate (`resolveScanResultCore` in `supabase/functions/claude-proxy/index.ts`)** is now the single place that decides whether a scan's HOT/LIST/SKIP, net profit, ROI, and max-buy-price may be computed at all — only when `marketDataSource: 'verified'`. When verification fails, the response reports `decisionAvailable: false`, `decisionStatus: 'insufficient_market_data'`, every authoritative field `null`, and the AI's own price/STR/days/demand guess only in a separate, clearly-labeled `aiEstimate` field that is never fed into `calcProfit`/`decide`/`calcMaxBuyPrice`. `apps/web/public/scanResultContract.js` validates this new shape (and fixes a latent validation bug where `decisionReasons` — always the full `DecisionResult` object from the server — was checked as if it were a plain string array); `app.html`'s `renderSingle`/`renderShelf` render a distinct "no verified recommendation" state instead of a HOT/LIST/SKIP badge whenever `decisionAvailable` is false.
