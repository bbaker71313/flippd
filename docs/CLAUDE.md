...
> Authoritative rules file for all Claude Code sessions.
...
1. `docs/HANDOFF.md` — what changed last session, current state
2. `docs/FEATURE_TRIAGE.md` — what to port vs build vs defer

Do NOT create duplicate files.
Do NOT recreate files that already exist — update them.
...
Run all 5 checks before writing a single line of code. Show output for each.
If any check fails, STOP and report. Do not proceed until resolved.
...
**STOP RULE:** If any check produces unexpected output, do not continue.
Document the failure in docs/HANDOFF.md and wait for instruction.
Do not guess. Do not self-fix without reporting first.
...
All values from Supabase project `dqgfpchkheznvanfgsmx`.
...
| Tier | Scans/mo | Items |
|---|---|---|
| trial | Unlimited | Unlimited (7 days) |
| scout | 25 | 10 |
| hustle | Unlimited | 500 |
...
| Tab | Feature |
|---|---|
| Scout | AI shelf scanner — single item + shelf mode |
| Inventory | Add/edit/delete items, status tracking, photos |
| Listing | AI listing generator, CSV export |
| Trends | Growth Agent — weekly business brief |
| Stats | P&L dashboard, expenses, mileage |

---

## 🤖 AI Prompts (port verbatim from Flippd — never rewrite)

All AI prompts live in `docs/FEATURE_TRIAGE.md` under "Port Directly".
When implementing any AI feature, extract the exact prompt from FEATURE_TRIAGE.md.
Do NOT rewrite, summarize, or improve the prompts — they are tested and trusted.
...
AI model: `claude-sonnet-4-6` — never downgrade without explicit instruction.
AI calls go through Supabase Edge Functions — never from client directly.
...
| Function | Replaces |
|---|---|
| `claude-proxy` | `/v1/messages` and `/v1/messages-with-image` |
| `auth` | `/auth/register`, `/auth/verify`, `/auth/login`, `/auth/me` |
...
Required coverage:
1. Happy path
2. Edge case (empty data, zero cost, no API response)
3. Failure case (API error, bad input, negative profit)
...
2. Read `docs/FEATURE_TRIAGE.md` — check port vs build vs defer
3. Read this file — done when you reach this line
4. Run SESSION START verification above — all 5 checks must pass

### During work:
- Karpathy Rule 3: surgical changes only — touch only what you must
...
- All copy through stop-slop rules before shipping
...
**If push fails:** Document exact error in `docs/HANDOFF.md`, leave working
tree with a local commit. Never end a session with uncommitted work and no record.
...
1. **Hardcoding fee percentages** — always from user settings via `ebayFee` param
2. **Hardcoding mileage rate or tax reserve** — always configurable in settings
3. **Using magic link auth** — it was removed. Email verification + password only
4. **Rewriting AI prompts** — port them verbatim from FEATURE_TRIAGE.md
5. **Adding a 6th tab** — 5 tabs only: Scout, Inventory, Listing, Trends, Stats
6. **Calling Anthropic API from client** — always via Supabase Edge Function
7. **Using StyleSheet in React Native** — NativeWind classes only
8. **Using `<form>` tags** — use onClick/onChange handlers
9. **Floating point currency math** — use precise arithmetic, store as numbers
10. **Storing dates without timezone** — always UTC ISO 8601
11. **Files over 500 lines** — refactor proactively
12. **Duplicating types** — all types in `packages/shared/src/types/index.ts` only
...
| 03 | Design | 🔄 Step 3 in progress |
| 04 | Build Mobile | ⬜ Not started |
| 05 | Build Web | ⬜ Not started |
...
| Step | Task | Status |
|---|---|---|
| 1 | Brand Identity → docs/BRAND_IDENTITY.md | ✅ Done |
| 2 | Design System → packages/shared/src/constants/theme.ts | ✅ Done |
| 3 | Component Library → apps/mobile/components/ui/ | 🔄 Redo with frontend-design skill |
| 4 | Screen Flows → Figma (Claude.ai + Figma MCP) | ⬜ Pending |
| 5 | Prototype → docs/prototype.html | ⬜ Pending |
