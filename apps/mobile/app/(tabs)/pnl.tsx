...
import { fetchStatsSummary, fetchExpenses, addExpense, createCheckoutSession } from '../../lib/stats'
...
export default function PnlScreen() {
  ...
  async function handleAddExpense() {
    ...
    try {
      ...
      setExpenses((prev: PnlExpense[]) => [exp, ...prev])
      setAddVisible(false)
      setExpAmount(''); setExpDesc(''); setExpMiles('')
      const sum = await fetchStatsSummary('all')
      setSummary(sum)
    } catch (e) {
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      >
        {/* Header — Change 20: branded header, no "Hey There", no Overview card */}
        <View style={{ paddingTop: SPACING.lg, paddingBottom: SPACING.md }}>
          <Text style={{ fontFamily: TYPOGRAPHY.h1.fontFamily, fontSize: 28, fontWeight: '700', color: COLORS.textPrimary }}>P&amp;L</Text>
          <Text style={{ fontFamily: TYPOGRAPHY.mono.fontFamily, fontSize: 13, color: COLORS.textMuted, marginTop: 2 }}>
            Your numbers, your business.
          </Text>
        </View>

        {/* ── 1. Full P&L breakdown ────────────────────────────────────────── */}
        {summary && (
          <View style={{ backgroundColor: COLORS.surface, borderRadius: RADIUS.lg, padding: SPACING.xl, borderWidth: 1, borderColor: COLORS.border, ...SHADOWS.sm, marginBottom: SPACING.md }}>
            ...
            {/* Net profit hero — full section, not a KPI card */}
            <Text style={{ fontFamily: TYPOGRAPHY.label.fontFamily, fontSize: TYPOGRAPHY.label.fontSize, color: COLORS.textMuted, marginBottom: SPACING.xs }}>NET PROFIT</Text>
            ...
            <Text style={{ fontFamily: TYPOGRAPHY.caption.fontFamily, fontSize: TYPOGRAPHY.caption.fontSize, color: COLORS.textMuted, marginBottom: SPACING.lg }}>
              ...
              {summary.itemsSold > 0 ? ` · ${summary.itemsSold} items sold` : ''}
            </Text>
            ...
            {/* Tax reserve callout — informational, not deducted */}
        ...
        {/* ── 2. Inventory snapshot — LISTED / UNLISTED only (no SOLD KPI) ── */}
        {summary && (
          <>
          ...
          <>
            ...
            {expenses.length === 0 ? (
              ...
              <View style={{ gap: SPACING.xs, marginBottom: SPACING.md }}>
                {expenses.slice(0, 10).map((exp: PnlExpense) => (
            ...
            {summary && summary.totalMileage > 0 ? (
              <View style={{ backgroundColor: COLORS.surface, borderRadius: RADIUS.md, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border, marginBottom: SPACING.sm }}>
                <Text style={{ fontFamily: TYPOGRAPHY.body.fontFamily, fontSize: TYPOGRAPHY.body.fontSize, color: COLORS.textSecondary }}>
                  ${summary.totalMileage.toFixed(2)} total deduction
                </Text>
              </View>
            ) : (
              <Text style={{ fontFamily: TYPOGRAPHY.caption.fontFamily, fontSize: TYPOGRAPHY.caption.fontSize, color: COLORS.textMuted, marginBottom: SPACING.sm }}>
